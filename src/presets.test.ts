...
describe('presets.ts', () => {
	...
	describe('initPresets', () => {
		...
		it('should initialize recName preset correctly', () => {
			const presets = initPresets(mockInstance)
			const recNamePreset = presets.recName

			expect(recNamePreset).toBeDefined()
			if (recNamePreset && recNamePreset.type === 'button') {
				expect(recNamePreset.type).toBe('button')
				expect(recNamePreset.category).toBe('Transport')
				expect(recNamePreset.name).toBe('Record w/ Name')
				expect(recNamePreset.style).toBeDefined()
				expect(recNamePreset.feedbacks).toHaveLength(0)
				expect(recNamePreset.steps).toHaveLength(1)
				expect(recNamePreset.steps[0].down).toHaveLength(1)
				expect(recNamePreset.steps[0].up).toHaveLength(0)
				expect(recNamePreset.steps[0].down[0]).toEqual({
					actionId: 'recName',
					options: { name: 'Recording' }
				})
			} else {
				fail('recNamePreset is not a button preset')
			}
		})

		it('should initialize recTimestamp preset correctly', () => {
			const presets = initPresets(mockInstance)
			const recTimestampPreset = presets.recTimestamp

			expect(recTimestampPreset).toBeDefined()
			if (recTimestampPreset && recTimestampPreset.type === 'button') {
				expect(recTimestampPreset.type).toBe('button')
				expect(recTimestampPreset.category).toBe('Transport')
				expect(recTimestampPreset.name).toBe('Record w/ Timestamp')
				expect(recTimestampPreset.style).toBeDefined()
				expect(recTimestampPreset.feedbacks).toHaveLength(0)
				expect(recTimestampPreset.steps).toHaveLength(1)
				expect(recTimestampPreset.steps[0].down).toHaveLength(1)
				expect(recTimestampPreset.steps[0].up).toHaveLength(0)
				expect(recTimestampPreset.steps[0].down[0]).toEqual({
					actionId: 'recTimestamp',
					options: { prefix: '' }
				})
			} else {
				fail('recTimestampPreset is not a button preset')
			}
		})
		...
		it('should initialize gotoN preset correctly', () => {
			const presets = initPresets(mockInstance)
			const gotoNPreset = presets.gotoN

			expect(gotoNPreset).toBeDefined()
			if (gotoNPreset && gotoNPreset.type === 'button') {
				expect(gotoNPreset.type).toBe('button')
				expect(gotoNPreset.category).toBe('Transport')
				expect(gotoNPreset.name).toBe('Goto Clip (n)')
				expect(gotoNPreset.style).toBeDefined()
				expect(gotoNPreset.style.text).toBe('Goto\nClip 1')
				expect(gotoNPreset.feedbacks).toHaveLength(1)
				expect(gotoNPreset.steps).toHaveLength(1)
				expect(gotoNPreset.steps[0].down).toHaveLength(1)
				expect(gotoNPreset.steps[0].up).toHaveLength(0)
				expect(gotoNPreset.steps[0].down[0]).toEqual({
					actionId: 'gotoN',
					options: { clip: 1, useVariable: false }
				})
			} else {
		...
		it('should initialize gotoName preset correctly', () => {
			const presets = initPresets(mockInstance)
			const gotoNamePreset = presets.gotoName

			expect(gotoNamePreset).toBeDefined()
			if (gotoNamePreset && gotoNamePreset.type === 'button') {
				expect(gotoNamePreset.type).toBe('button')
				expect(gotoNamePreset.category).toBe('Transport')
				expect(gotoNamePreset.name).toBe('Goto Clip (name)')
				expect(gotoNamePreset.style).toBeDefined()
				expect(gotoNamePreset.style.text).toBe('Goto\nNamed')
				expect(gotoNamePreset.feedbacks).toHaveLength(1)
				expect(gotoNamePreset.steps).toHaveLength(1)
				expect(gotoNamePreset.steps[0].down).toHaveLength(1)
				expect(gotoNamePreset.steps[0].up).toHaveLength(0)
				expect(gotoNamePreset.steps[0].down[0]).toEqual({
					actionId: 'gotoName',
					options: { clip: '' }
				})
			} else {
		...
		it('should initialize goFwd preset correctly', () => {
			const presets = initPresets(mockInstance)
			const goFwdPreset = presets.goFwd

			expect(goFwdPreset).toBeDefined()
			if (goFwdPreset && goFwdPreset.type === 'button') {
				expect(goFwdPreset.type).toBe('button')
				expect(goFwdPreset.category).toBe('Transport')
				expect(goFwdPreset.name).toBe('Go Fwd Clip')
				expect(goFwdPreset.style).toBeDefined()
				expect(goFwdPreset.style.text).toBe('Next\nClip')
				expect(goFwdPreset.feedbacks).toHaveLength(0)
				expect(goFwdPreset.steps).toHaveLength(1)
				expect(goFwdPreset.steps[0].down).toHaveLength(1)
				expect(goFwdPreset.steps[0].up).toHaveLength(0)
				expect(goFwdPreset.steps[0].down[0]).toEqual({
					actionId: 'goFwd',
					options: { clip: 1, useVariable: false }
				})
			} else {
				fail('goFwdPreset is not a button preset')
			}
		})

		it('should initialize goRew preset correctly', () => {
			const presets = initPresets(mockInstance)
			const goRewPreset = presets.goRew

			expect(goRewPreset).toBeDefined()
			if (goRewPreset && goRewPreset.type === 'button') {
				expect(goRewPreset.type).toBe('button')
				expect(goRewPreset.category).toBe('Transport')
				expect(goRewPreset.name).toBe('Go Rew Clip')
				expect(goRewPreset.style).toBeDefined()
				expect(goRewPreset.style.text).toBe('Prev\nClip')
				expect(goRewPreset.feedbacks).toHaveLength(0)
				expect(goRewPreset.steps).toHaveLength(1)
				expect(goRewPreset.steps[0].down).toHaveLength(1)
				expect(goRewPreset.steps[0].up).toHaveLength(0)
				expect(goRewPreset.steps[0].down[0]).toEqual({
					actionId: 'goRew',
					options: { clip: 1, useVariable: false }
				})
			} else {
				fail('goRewPreset is not a button preset')
			}
		})

		it('should initialize fetchClips preset correctly', () => {
			const presets = initPresets(mockInstance)
			const fetchClipsPreset = presets.fetchClips

			expect(fetchClipsPreset).toBeDefined()
			if (fetchClipsPreset && fetchClipsPreset.type === 'button') {
				expect(fetchClipsPreset.type).toBe('button')
				expect(fetchClipsPreset.category).toBe('Utility')
				expect(fetchClipsPreset.name).toBe('Fetch Clips')
				expect(fetchClipsPreset.style).toBeDefined()
				expect(fetchClipsPreset.style.text).toBe('Fetch\nClips')
				expect(fetchClipsPreset.feedbacks).toHaveLength(0)
				expect(fetchClipsPreset.steps).toHaveLength(1)
				expect(fetchClipsPreset.steps[0].down).toHaveLength(1)
				expect(fetchClipsPreset.steps[0].up).toHaveLength(0)
				expect(fetchClipsPreset.steps[0].down[0]).toEqual({
					actionId: 'fetchClips',
					options: {}
				})
			} else {
				fail('fetchClipsPreset is not a button preset')
			}
		})
